...
save_name = strcat('output\APPEN\SIM_UnknownGroup_K',num2str(G0),'_param',num2str(parchoice),'_LargeN.mat');
save(save_name);


%% RMSE
[reshape(mean(cell2mat(dataholder{1,3})),10,2)...
    reshape(mean(cell2mat(dataholder{2,3})),10,1)]


%% Estmated Group Number - IC
[mean(dataholder{1,5}) mean(dataholder{2,5})]


%% 

% large NT
tmp = cell2mat(dataholder{1,4});
pen = ([1:8].*(H+1)+N).*((T-H)^(-0.5));
ictmp = tmp+tmp(:,end).*pen;
[~,Gtmp] = min(ictmp,[],2);
strcat('N=',num2str(N),', T=',num2str(T),', G=',num2str(mean(Gtmp)))

% large N
tmp = dataholder{1,4};
for tt = 1:2
    tmp1 = cell2mat(tmp(:,tt));
    pen = ([1:8].*(H+1)+N).*((Tgrid(tt)-H)^(-0.5));
    ictmp = tmp1+tmp1(:,end).*pen;
    [~,Gtmp] = min(ictmp,[],2);
    strcat('N=',num2str(N),', T=',num2str(Tgrid(tt)),', G=',num2str(mean(Gtmp)))
end
...
for jj = 1:2
    ...
    tmp1 = cell2mat(tmp(:,1));
    % alternative
    pen = ([1:8].*(H+1)+Ngrid(jj)).*((T-H)^(-0.5));
    ictmp = tmp1+tmp1(:,end).*pen;
    [~,Gtmp] = min(ictmp,[],2);
    strcat('N=',num2str(Ngrid(jj)),', T=',num2str(T),', G=',num2str(mean(Gtmp)))
end
