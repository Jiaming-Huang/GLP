...
save_name = strcat('output\APPEN\SIM_LargeT_FE_noc.mat');
save(save_name);
...
[mean(reshape(mean(cell2mat(dataholder{1,1,2})),H+1,3));
    mean(reshape(mean(cell2mat(dataholder{2,1,2})),H+1,3));
    mean(reshape(mean(cell2mat(dataholder{1,2,2})),H+1,3));
    mean(reshape(mean(cell2mat(dataholder{2,2,2})),H+1,3));]
