...
for jj = 1:NGridSize
    ...
    % GLP - HBH
    GLP_GR_HBH       = cell(nRep,TGridSize); % GLP Gr_hat
    GLP_AC_HBH       = nan(nRep, TGridSize); % GLP classification accuracy
    GLP_IR_HBH       = cell(nRep,TGridSize); % GLP IR
    GLP_SE_HBH       = cell(nRep,TGridSize); % GLP standard errors
    GLP_MSE_HBH      = nan(nRep, TGridSize); % GLP mean squared errors
    GLP_BR_HBH       = nan(nRep,TGridSize); % GLP confidence band ratio (relative to IND_LP)
    GLP_CP_HBH       = cell(nRep,TGridSize); % GLP coverage probability
    ...
    for tt = 1:TGridSize
        ...
        parfor iRep = 1:nRep
            ...
            Ubands        = GIRF_re + 1.96*GSE_re;
            Lbands        = GIRF_re - 1.96*GSE_re;
            GLP_GR_HBH{iRep,tt} = Gr_re;
            GLP_AC_HBH(iRep,tt) = mean(AC); 
            GLP_IR_HBH{iRep,tt} = GIRF_re;
            GLP_SE_HBH{iRep,tt} = GSE_re;
            GLP_MSE_HBH(iRep,tt)= mean(MSE);
            GLP_BR_HBH(iRep,tt) = mean(BR);
            GLP_CP_HBH{iRep,tt} = (Ubands > IR_true) & (Lbands < IR_true);
    ...
    % Store outputs
    % Group membership
    dataholder{jj,1}  = [GLP_GR, GLP_GR_HBH]; % nRep x TGridSize x 1 cell
    % Accuracy
    dataholder{jj,2}  = [GLP_AC, GLP_AC_HBH]; % nRep x TGridSize x 1 matrix
    % IRs
    dataholder{jj,3}  = [GLP_IR, GLP_IR_HBH]; % nRep x TGridSize x 3 cell
    % Stanrd errors
    dataholder{jj,4}  = [GLP_SE, GLP_SE_HBH]; % nRep x TGridSize x 3 cell
    % MSE
    dataholder{jj,5}  = [GLP_MSE, GLP_MSE_HBH, PAN_MSE, IND_MSE, IGLP_MSE]; % nRep x TGridSize x 4 matrix
    % Band ratios
    dataholder{jj,6}  = [GLP_BR, GLP_BR_HBH, IGLP_BR]; % nRep x TGridSize x 2 matrix
    % Coverage probabilities
    dataholder{jj,7}  = GLP_CP; % nRep x TGridSize x G0 by H matrix
    dataholder{jj,8}  = GLP_CP_HBH;
    dataholder{jj,9}  = IGLP_CP;
...
%% TABLES
%% Accuracy
Accuracy = round([reshape(mean(dataholder{1,2}),TGridSize,2);
    reshape(mean(dataholder{2,2}),TGridSize,2);
    reshape(mean(dataholder{3,2}),TGridSize,2)],4);
disp(Accuracy)
%% RMSE for GLP (col 1), PAN (col 2), IND (col 3), IGLP (col 4)
RMSE = round(sqrt([reshape(mean(dataholder{1,5}),TGridSize,5);
    reshape(mean(dataholder{2,5}),TGridSize,5);
    reshape(mean(dataholder{3,5}),TGridSize,5)]),4);
disp(RMSE)
%% BR for - GLP IGLP
BR = round([reshape(mean(dataholder{1,6}),TGridSize,3);
    reshape(mean(dataholder{2,6}),TGridSize,3);
    reshape(mean(dataholder{3,6}),TGridSize,3)],4);
disp(BR)
%% Coverage GLP - AsymV
...
disp(CP_GLP)
%% Coverage GLP - IGLP
CP_GLP_HBH = array2table(round([reshape(mean(mean(cell2mat(dataholder{1,8})),3),TGridSize,H+1);
...
disp(CP_GLP_HBH)
%% Coverage GLP - IGLP
CP_IGLP = array2table(round([reshape(mean(mean(cell2mat(dataholder{1,9})),3),TGridSize,H+1);
