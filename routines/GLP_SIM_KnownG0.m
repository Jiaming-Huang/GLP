function [Gr_EST, GIRF, GSE] = GLP_SIM_KnownG0(reg, G0, bxInit, bcInit, weight, FE, inference)
% GLP algorithm used for Section 6.2, S1.1, S1.3, S1.5, S1.6, S1.7, S1.8
% we make two simplifying settings:
%       1) we initialize with true IRs and just run one iteration -- simply to speed up
%       2) we take the true number of groups as given instead of selecting it using IC
% We provide both mixed weighting scheme and raw weighting
...
% GLP_SIM_KnownG0_Inference.m: this function computes both large T and fixed T SEs
...
%   weight: can be string ('2SLS', 'IV') or user-supplied weighting;
%           default: mixed weighting (unit-horizon specific & identity)
%                   see Sec S1.3 for details
%   FE: 1 - fixed effects (within estimator, demean)
%   inference: 1 - large T (mixed with IV); 
%              2 - fixed T (mixed with IV); 
%              3 - large T (raw weight)
%              4 - fixed T (raw weight)
...
if G0 == 1   % Trivial case, single group
    ...
    GIRF = pagemldivide(xzrmrzx_tmp,xzrmrzy_tmp);
    Gr_EST = ones(N,1);
...
%% Inference
if inference == 1
    %% Large T, mixed with IV weighting matrix
    GSE = nan(K,1,G0,H);
    ...
    czozy = pagemtimes(czo,zy);
    czozx = pagemtimes(czo,zx);
    M = eye(L) - pagemtimes(pagemrdivide(rzc,czozc),'none',rzc,'transpose');
    rzx = pagemtimes(R,zx);
    xzr = pagetranspose(rzx);
    rzy = pagemtimes(R,zy);
    ...
    for g = 1:G0
        ...
        xzrmrzy_tmp = sum(xzrmrzy(:,:,Gr_EST==g,:),3);
...
elseif inference == 2
    %% Fixed T, , mixed with IV weighting matrix
    %     zz  = pagemtimes(zp,'none',zp,'transpose')/T; % L by L by N
    %     OMEGA = repmat(pageinv(zz),1,1,1,H);
    %     [U,S,~]=pagesvd(OMEGA,'vector');
    %     R = sqrt(S).*pagectranspose(U);
    ...
    for j = 1:G0
        ...
        part_jl = xzrmrzy - pagemtimes(xzrmrzx,GIRF(:,:,j,:));
        for k = 1:G0
            if j==k
                f_ijg = zeros(1,G0,N);
                ...
                for g = setdiff([1:G0],j)
                    ...
                    dist = reshape(sum(pagemtimes(IR_dif,'transpose',2*xzrmrzy - pagemtimes(xzrmrzx,GIRF(:,:,j,:)+GIRF(:,:,g,:)),'none'),4),N,1);
                ...
                dist = reshape(sum(pagemtimes(IR_dif,'transpose',2*xzrmrzy - pagemtimes(xzrmrzx,GIRF(:,:,j,:)+GIRF(:,:,k,:)),'none'),4),N,1);
                bw   = std(dist)*1.06*N^(-0.2);
                f_ijk = nan(1,1,N);
    ...
    GSE = permute(reshape(diag(sqrt(Gam\V/Gam)),K,H,G0),[1,4,3,2]);
