...
for Ghat = 1:Gmax
    ...
    %% Default: re-estimate using identity weight & estimated groups
...
%         e_tmp = yp_tmp - xb - cphi;
% 
%         ze_tmp = pagemtimes(zp(:,:,gr_est==g),e_tmp);
%         tmp = pagemtimes(pagemtimes(ze_tmp,'transpose',OMEGA1(:,:,gr_est==g,:),'none'),ze_tmp);
%         obj(g) = sum(tmp(:))/(T^2); % divide by N at the end
    end
%     Q = sum(obj)/N;
