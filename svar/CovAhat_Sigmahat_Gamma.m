...
% size(kron([zeros(n*p,m),eye(n*p)]*(Q1^(-1)),eye(n)),2)+...
% size(zeros((n^2)*p,n^2+(k*n)),2)
% 
% size(zeros(n*(n+1)/2,((n^2)*p)+n*m),2)+...
% size(V,2)+...
% size(zeros(n*(n+1)/2,k*n),2)
% 
% size( -kron(Q2*(Q1^(-1)),eye(n)),2)+...
% size(zeros(k*n,n^2),2)+...
% size(eye(k*n),2)
