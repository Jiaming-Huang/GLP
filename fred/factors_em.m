function [ehat,Fhat,lamhat,ve2,x2,mut,sdt] = factors_em(x,kmax,jj,DEMEAN)
% =========================================================================
% DESCRIPTION
...
%           mut     = the mean of x
%           sdt     = the standard deviation of x
%
% Note: ehat has been adjusted so that x = mut + Fhat*lamhat'.*sdt + ehat;
%       so to recover IRFs of info vars we need only scale IRFs by sdt
% -------------------------------------------------------------------------
% SUBFUNCTIONS
%
...
% Check that jj is one of 1, 2, 3, 4
if jj>5
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function r=onatski(X,rmax)
% =========================================================================
% DESCRIPTION
% This function determines the number of factors to be selected for a given
% dataset following Onatski (2010)
%
% -------------------------------------------------------------------------
% INPUTS
%           X       = dataset (one series per column)
...
% Part 1: Setup.
%
% Part 2: Compute the eigenvalues of the sample covariance matrix and set
%         j = kmax + 1.
%
% Part 3: compute the slope beta in the regression of D(j)...D(j+4) on the
%         constant and (j-1)^(2/3)...(j+3)^(2/3). Set delta=2*abs(beta)
%
% Part 4: Save other output variables to be returned by the function (chat,
%         Fhat, and eigval).
%
...
beta    = (indepen'*indepen)\(indepen'*depen);
